...
@section('content')
    <div class="content" id="mydiv">
        <div class="row page-title-div">
            <div class="col-md-10">
                <h2 class="title">  {{ \Illuminate\Support\Facades\DB::table('exams')->where('id', $exam)->value('name') }} Term
                    Tsheet of Class {{ \Illuminate\Support\Facades\DB::table('classes')->where('id', $class)->value('classname') }}</h2>
                ...
                <table style="width: 100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$mak->prac}}</td>
                            <td>{{$mak->theory}}</td>
                            <td>{{ \Illuminate\Support\Facades\DB::table('grades')->where('id', $mak->grade_id)->value('gpoint') }}</td>
                            {{-- <td>{{$mak->grade_id}}</td> --}}
                            @endif
                             @endif
                             @endforeach
                             ...
                             @if($mak->class_id==$class && $mak->exam_id==$exam)
                             <td>
                             {{\Illuminate\Support\Facades\DB::table('marks')->where('student_id', $t->student_id)->sum('total') }}</td>
                             {{-- <td>   {{$mak->grade_id}}  </td> --}}
                             <td>{{ \Illuminate\Support\Facades\DB::table('grades')->where('id', $mak->grade_id)->value('name') }}</td>
                             <td>{{$row1++}}</td>
                             @endif
                              @endif
                              @endforeach
